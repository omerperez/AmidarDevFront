// import CreateMeetingDialog from "../../Features/HomePageFeatures/CreateMeetingDialog";
...
class Column {
  ...
  type: string = "";
  width: Number = ((window.innerWidth * 0.97) / 14) * 0.75;
  renderCell: any;
  ...
  constructor(field: string, headerName: string) {
    ...
    if (field === "createMeeting") {
      this.sortable = false;
      this.disableColumnMenu = true;
      this.disableColumnFilter = true;
      this.disableColumnSelector = true;
      this.type = "actions";
      this.renderCell = (params: any) => {
        // const currentTenant = new TenantRow(params.row);
        // return <CreateMeetingDialog currentTenant={currentTenant} />;
      };
    }
    if (field === "map") {
      this.sortable = false;
...
export { TableColumns, TableRow, MainTenantTableDetails };
